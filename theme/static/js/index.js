...
function build(providerSpec, log, fitAddon, path, pathType) {
  ...
  image.onStateChange('failed', function() {
    ...
    if ($('div#loader-text').length > 0) {
      ...
      $('div#loader-text p.launching').html('Error loading ' + spec + '!<br /> See logs below for details. If you already have a server running click <a href="{{ JUPYTERHUB_URL }}/hub/home">here</a>.');
    }
    image.close();
  });
